...
public class AudioController : MonoBehaviour
{
    ...
    public void fadingBGM(bool fadeIn, float fadeDuration = 1f)
    {
        if (this.bgmAudioSource != null && this.audioStatus)
        {
            if (fadeIn)
            {
                // Fade in
                if(this.bgmAudioSource.volume == 0f)
                {
                    this.bgmAudioSource.DOFade(0.2f, fadeDuration);
                }
            }
            else
            {
                ...
                if (this.bgmAudioSource.volume > 0f)
                {
                    this.bgmAudioSource.DOFade(0f, fadeDuration);
                }
            }
        }
    }
